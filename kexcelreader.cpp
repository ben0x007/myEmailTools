...
bool KExcelReader::open( const QString& xlsFile )
{
    ...
    if (m_workbooks && !m_workbooks->isNull())
    {
       ...
       // m_workbook = m_workbooks->querySubObject("Open(const QString&)", "C:/Users/Thinkpad/Desktop/aa.xlsx");
    }
...
/*
QAxObject* KExcelReader::getSheets() const
{
    return m_sheets;
}
*/
QAxObject* KExcelReader::getWorkbook() const
{
   return m_workbook;
}
...
QList<QVariantList> KExcelReader::values(int colsCount, int rowsCount, int sheetNumber) const
{
    ...
    for (int row=1; row <= rowsCountToRead; row++)
    {
        ...
        for (int column=1; column <= colsCountToRead; column++)
        {
            ...
            // qDebug("string value:%s", value.toString());
            //qDebug()<<"value:"+value.toString();

           /*
           if(value.canConvert<double>()){
             qDebug("double value:%d", value.toDouble());
           }*/
